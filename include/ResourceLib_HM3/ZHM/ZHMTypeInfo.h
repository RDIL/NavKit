...
private:
    struct PrimitiveRegistrar {
        PrimitiveRegistrar() {
            IZHMTypeInfo::RegisterPrimitiveTypes();
        }
    };
...
public:
    ZHMTypeInfo(const char* p_TypeName, size_t p_Size, size_t p_Alignment, WriteTypeAsJson_t p_WriteSimpleJson,
        CreateTypeFromJson_t p_CreateFromJson, SerializeType_t p_Serialize, CheckEquals_t p_CheckEquals = nullptr,
        DestroyObject_t p_DestroyObject = nullptr) :
        m_Name(p_TypeName), m_Size(p_Size), m_Alignment(p_Alignment), m_WriteSimpleJson(p_WriteSimpleJson),
        m_CreateFromJson(p_CreateFromJson), m_Serialize(p_Serialize), m_CheckEquals(p_CheckEquals),
