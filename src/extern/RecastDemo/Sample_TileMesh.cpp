...
Sample_TileMesh::Sample_TileMesh() :
    m_keepInterResults(false), m_buildAll(true), m_totalBuildTimeMs(0), m_triareas(0), m_solid(0), m_chf(0), m_cset(0),
    m_pmesh(0), m_dmesh(0), m_drawMode(DRAWMODE_NAVMESH), m_maxTiles(0), m_maxPolysPerTile(0), m_tileSize(64),
    m_tileCol(duRGBA(0, 0, 0, 32)), m_tileBuildTime(0), m_tileMemUsage(0), m_tileTriCount(0) {
    resetCommonSettings();
...
void Sample_TileMesh::handleRender() {
    ...
    if (m_navMesh && m_navQuery &&
        (m_drawMode == DRAWMODE_NAVMESH || m_drawMode == DRAWMODE_NAVMESH_TRANS ||
            ...
            m_drawMode == DRAWMODE_NAVMESH_PORTALS || m_drawMode == DRAWMODE_NAVMESH_INVIS)) {
...
void Sample_TileMesh::buildAllTiles() {
    ...
    Logger::log(NK_INFO,
        ("Building tiles. Number of rows (y): " + std::to_string(th) + ". Number of columns (x):" + std::to_string(tw))
            .c_str());
...
unsigned char* Sample_TileMesh::buildTileMesh(
    ...
    // neighbours, or use the bounding box below to only pass in a sliver of each of the 8 neighbours.
    ...
    for (int i = 0; i < m_geom->getConvexVolumeCount(); ++i)
        rcMarkConvexPolyArea(
            m_ctx, vols[i].verts, vols[i].nverts, vols[i].hmin, vols[i].hmax, (unsigned char)vols[i].area, *m_chf);
