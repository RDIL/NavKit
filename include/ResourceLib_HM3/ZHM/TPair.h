...
template <typename T, typename Z> class TPair {
public:
    ...
    static void FromSimpleJson(simdjson::ondemand::value p_Document, void* p_Target) {
        ...
        for (simdjson::ondemand::value s_PairValue : p_Document) {
            if (s_First) {
                if constexpr (std::is_same_v<T, int8> || std::is_same_v<T, uint8> || std::is_same_v<T, int16> ||
            ...
            if constexpr (std::is_same_v<Z, int8> || std::is_same_v<Z, uint8> || std::is_same_v<Z, int16> ||
                std::is_same_v<Z, uint16> || std::is_same_v<Z, int32> || std::is_same_v<Z, uint32>) {
                int64_t s_Value = s_PairValue;
...
#if ZHM_TARGET != 2012
    static_assert((((sizeof(T) + 7) & (-8)) - sizeof(T)) == 0,
        "First type of TPair has an unaligned size. This is currently not supported.");
#else
    static_assert((((sizeof(T) + 3) & (-4)) - sizeof(T)) == 0,
        "First type of TPair has an unaligned size. This is currently not supported.");
#endif
};
